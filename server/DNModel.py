...
def buildNetwork(networkBlocks):
    ...
    for i, x in enumerate(networkBlocks[1:]):
        ...
        if (x["type"] == "convolutional"):
            ...
            conv = nn.Conv2d(channels, filters, kernelSize,
            ...
            anchors = [(anchors[j], anchors[j+1])
                       for j in range(0, len(anchors), 2)]
    ...
    def load_weights(self, weightfile):
        ...
        for i in range(len(self.moduleList)):
            ...
            if module_type == "convolutional":
                ...
                try:
                    batch_normalize = int(
                        self.netBlocks[i+1]["batch_normalize"])
                except:
                    batch_normalize = 0
                ...
                if (batch_normalize):
                    ...
                    bnBias = torch.from_numpy(
                        weights[tracker:tracker + biasCount])
                    tracker += biasCount

                    bnPart_weights = torch.from_numpy(
                        weights[tracker: tracker + biasCount])
                    tracker += biasCount

                    bnPart_running_mean = torch.from_numpy(
                        weights[tracker: tracker + biasCount])
                    tracker += biasCount

                    bnPart_running_var = torch.from_numpy(
                        weights[tracker: tracker + biasCount])
                    tracker += biasCount

                    bnBias = bnBias.view_as(bnPart.bias.data)
                    bnPart_weights = bnPart_weights.view_as(bnPart.weight.data)
                    bnPart_running_mean = bnPart_running_mean.view_as(
                        bnPart.running_mean)
                    bnPart_running_var = bnPart_running_var.view_as(
                        bnPart.running_var)
                ...
                convWeight = torch.from_numpy(
                    weights[tracker:tracker+weightfile])
...
'''
#Test CFG:
construct = construct_cfg('yolov3cfg/yolov3.cfg')
